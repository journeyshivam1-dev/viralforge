...
interface PublishReelInstagramRequest {
  accountId: string; // Instagram Professional Account ID
  videoUrl: string;
  caption: string;
...
export class InstagramPublisher {
   ...
   */
  async publishReel(request: PublishReelInstagramRequest): Promise<InstagramPublishResult> {
    try {
      // Step 1: Create media container
      const containerResponse = await fetch(
        `${this.baseUrl}/${request.accountId}/media`,
        {
          method: 'POST',
      ...
      const publishResponse = await fetch(
        `${this.baseUrl}/${request.accountId}/media_publish`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
   ...
   */
  private async pollContainerStatus(creationId: string, maxAttempts = 30): Promise<void> {
    for (let i = 0; i < maxAttempts; i++) {
      const response = await fetch(
        `${this.baseUrl}/${creationId}?fields=status_code,status&access_token=${this.accessToken}`
      );
   ...
   */
  async getAccountInsights(mediaId: string): Promise<any> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${mediaId}/insights?metric=impressions,reach,engagement,saves,shares&access_token=${this.accessToken}`
      );
...
export class FacebookPublisher {
   ...
   */
  async publishReel(request: PublishReelFacebookRequest): Promise<FacebookPublishResult> {
    try {
      // Upload video to Facebook
      const uploadResponse = await fetch(
        `${this.baseUrl}/${request.pageId}/video_reels`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
   ...
   */
  private async pollVideoStatus(videoId: string, maxAttempts = 30): Promise<void> {
    for (let i = 0; i < maxAttempts; i++) {
      const response = await fetch(
        `${this.baseUrl}/${videoId}?fields=status&access_token=${this.accessToken}`
      );
...
export class MetaOAuth {
  ...
  private baseUrl = 'https://graph.facebook.com/v18.0';
  private dialogUrl = 'https://www.facebook.com/v18.0/dialog/oauth';
   ...
   */
  async getInstagramAccount(pageId: string, accessToken: string): Promise<any | null> {
    const response = await fetch(
...
export function createMetaOAuth(
  appId: string,
  appSecret: string,
  redirectUri: string
): MetaOAuth {
